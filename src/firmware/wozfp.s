...
CONT:
	JSR	SWAP                         ; MOVE ARG TO EXP/MANT2
	LDX #0                           ; LOAD X FOR HIGH BYTE OF EXPONENT
	LDA	X2                           ; HOLD EXPONENT
	LDY	#0x80
	STY	X2                           ; SET EXPONENT 2 TO 0 (0x80)
	EOR	#0x80                         ; COMPLIMENT SIGN BIT OF ORIGINAL EXPONENT
	STA	M1+1                         ; SET EXPONENT INTO MANTISSA 1 FOR FLOAT
	BPL CONT1                        ; IS EXPONENT NEGATIVE
	DEX                              ; YES, SET X TO 0xFF
...
LN10:
	.BYTE 0x7E, 0x6F, 0x2D, 0xED		; 0.4342945
R22:
	.BYTE 0x80, 0x5A, 0x82, 0x7A		; 1.4142136, SQRT(2)
LE2:
	.BYTE 0x7F, 0x58, 0xB9, 0x0C		; 0.69314718, LOG BASE E OF 2
A1:
	.BYTE 0x80, 0x52, 0xB0, 0x40		; 1.2920074
MB:
	.BYTE 0x81, 0xAB, 0x86, 0x49		; -2.6398577
C:
	.BYTE 0x80, 0x6A, 0x08, 0x66		; 1.6567626
MHLF:
	.BYTE 0x7F, 0x40, 0x00, 0x00		; 0.5
...
L2E:
	.BYTE 0x80, 0x5C, 0x55, 0x1E		; 1.4426950409, LOG BASE 2 OF E
A2:
	.BYTE 0x86, 0x57, 0x6A, 0xE1		; 87.417497202
B2:
	.BYTE 0x89, 0x4D, 0x3F, 0x1D		; 617.9722695
C2:
	.BYTE 0x7B, 0x46, 0xFA, 0x70		; 0.03465735903
D:
	.BYTE 0x83, 0x4F, 0xA3, 0x03		; 9.9545957821
...
SWAP:
	LDX	#0x04                    ; INDEX FOR 4-BYTE SWAP.
...
RTAR:
	LDA	M1                      ; SIGN OF MANT1 INTO CARRY FOR
	ASL				; RIGHT ARITH SHIFT

RTLOG:
	INC	X1                     ; INCR EXP1 TO COMPENSATE FOR RT SHIFT
	BEQ	OVFL                         ; EXP1 OUT OF RANGE.

RTLOG1:
	LDX	#0xFA                  ; INDEX FOR 6 BYTE RIGHT SHIFT
...
FCOMPL:
	...
	LDX	#0x03                         ; INDEX FOR 3 BYTE SUBTRACTION
...
DIV1:
	SEC                         ; SET CARRY FOR SUBTRACT
	LDX	#0x02                         ; INDEX FOR 3-BYTE INSTRUCTION
...
DIV2:
	...
	LDX	#0xFD                         ; INDEX FOR 3-BYTE CONDITIONAL MOVE
...
MD3:
	EOR	#0x80                     ; COMPLIMENT SIGN BIT OF EXP
	STA	X1                           ; STORE IT
	LDY	#0x17                         ; COUNT FOR 24 MUL OR 23 DIV ITERATIONS
	RTS                              ; RETURN
